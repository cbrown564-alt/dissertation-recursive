# S3 YAML-To-JSON Extraction With Evidence

You are extracting structured information from an epilepsy clinic letter.

Return only YAML that can be parsed into one object matching the canonical
extraction schema after YAML-to-JSON conversion. Do not wrap the response in
Markdown. Use `pipeline_id: "S3_yaml_to_json_evidence"`.
