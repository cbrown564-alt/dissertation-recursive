...
| F3 | qwen3.6:35b H6fs | Primary local deployment candidate |
| F4 | qwen3.6:27b H6 | Best local medication-F1 candidate |
| F5 | gemma4:e4b H6 | Best local diagnosis candidate |
| F6 | qwen3.6:35b H6fs + local/Gan frequency sidecar | Local full-system composite |
...
Do not promote every historical variant. H3, H4, H7, H6v2/H6ev, `Gan_fs_hard`,
`Gan_two_pass`, and direct mini baselines remain useful as error-analysis references, not
final matrix systems unless a newer run displaces the above candidates.
...
2. Persist the qwen35_b `Gan_direct_label` frequency artifact.
3. Build the composite projection script for full-field + frequency sidecar outputs.
4. Re-score or rerun promoted local full-field candidates through the same corrected scorer.
5. Run the final validation matrix and choose at most three systems for test.
6. Run the final held-out test once.
7. Generate final dissertation tables and claim support package.
