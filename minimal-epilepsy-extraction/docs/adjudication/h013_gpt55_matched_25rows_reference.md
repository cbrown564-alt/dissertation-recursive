# Adjudication Reference — 20260501T180000Z_h013_production_multi_agent_llm_gpt_5_5_n50
...
**SF** gold=`2 cluster per month, 6 per cluster` | pred=`2 cluster per month, 6 per cluster` | exact=✓ monthly=✓
  **med** `sodium valproate 500 mg bd` status=current | ev: _Sodium valproate 500 mg bd_
  **med** `clobazam 10 mg nocte (as rescue on cluster days)` status=current | ev: _Clobazam 10 mg nocte (as rescue on cluster days)_
  **sz_type** `Generalised epilepsy` onset=generalized | ev: _Generalised epilepsy with documented generalised spike-and-wave activity_
  **sz_type** `brief generalised convulsions` onset=generalized | ev: _brief generalised convulsions with rapid recovery_
  **inv** `EEG` result=abnormal | ev: _documented generalised spike-and-wave activity on prior EEG (2017)_
  **inv** `EEG` result=not_stated | ev: _Consider ambulatory EEG if frequency or pattern worsens_
  **inv** `other` result=normal | ev: _Liver function tests in August were within normal limits_
...
**SF** gold=`6 per 7 month` | pred=`seizure free since late August` | exact=✗ monthly=✗
  **med** `Lamotrigine 150 mg b.d.` status=current | ev: _Current treatment consists of Lamotrigine 150 mg b.d._
  **med** `Levetiracetam 1 g b.d.` status=current | ev: _Levetiracetam 1 g b.d._
  **sz_type** `focal epilepsy` onset=focal | ev: _known to our service with focal epilepsy_
  **sz_type** `prolonged focal seizure` onset=focal | ev: _prolonged focal seizure lasting 8 minutes_
  **sz_type** `brief absences` onset=generalized | ev: _four further brief absences_
  **sz_type** `generalised tonic-clonic seizure` onset=generalized | ev: _a single generalised tonic-clonic seizure_
...
**SF** gold=`2 per 5 month` | pred=`2 per 5 month` | exact=✓ monthly=✓
  **med** `Sodium valproate 500 mg twice daily` status=current | ev: _Sodium valproate 500 mg twice daily_
  **med** `Clobazam 10 mg at night as needed for jet-lag periods` status=current | ev: _Clobazam 10 mg at night as needed for jet-lag periods_
  **sz_type** `generalised epilepsy` onset=generalized | ev: _Patient has generalised epilepsy._
  **sz_type** `nocturnal events with rhythmic twitching of the right arm and déjà vu` onset=focal | ev: _at night while asleep. He woke with rhythmic twitching of the right arm and a sense of déjà vu_
  **inv** `EEG` result=not_stated | ev: _Arrange routine EEG at next in-person visit when in London_
  **inv** `EEG` result=not_stated | ev: _if logistics difficult, consider ambulatory EEG coordinated around his travel_
...
**SF** gold=`4 per day` | pred=`4 per day; 2 per month` | exact=✗ monthly=✗
  **sz_type** `focal onset seizures` onset=focal | ev: _focal onset seizures four times per day_
  **sz_type** `drop attacks` onset=unknown | ev: _drop attacks occurring in batches_
  **sz_type** `tonic-clonic seizures` onset=generalized | ev: _tonic-clonic seizures 2 times per month_
  **sz_type** `brief lapses with a rising internal sensation and impaired awareness` onset=focal | ev: _brief lapses with a rising internal sensation and impaired awareness lasting under a minute_
...
**SF** gold=`multiple per week` | pred=`several per week` | exact=✗ monthly=✗
  **med** `Sodium valproate 500 mg twice daily` status=current | ev: _Medication: Sodium valproate 500 mg twice daily._
  **sz_type** `brief generalised absence-type seizures` onset=generalized | ev: _brief generalised absence-type seizures_
  **sz_type** `morning myoclonic jerks on awakening` onset=generalized | ev: _occasional morning myoclonic jerks on awakening_
  **sz_type** `startle-triggered events` onset=generalized | ev: _Generalised epilepsy with photosensitivity and startle-triggered events_
  **inv** `MRI` result=normal | ev: _MRI brain (2023) was normal._
  **inv** `EEG` result=abnormal | ev: _EEG (2024) showed generalised 3 Hz spike-and-wave discharges provoked by hyperventilation and intermittent photic stimul_
  **inv** `other` result=normal | ev: _Routine bloods last month were normal including FBC and LFTs._
  **inv** `EEG` result=not_stated | ev: _if needed, repeat EEG with sleep deprivation_
...
**SF** gold=`4 per 6 week` | pred=`5 per 6 weeks` | exact=✗ monthly=✗
  **med** `Lamotrigine 150 mg twice daily` status=current | ev: _Lamotrigine 150 mg twice daily_
  **med** `Clobazam 10 mg at night as needed` status=current | ev: _Clobazam 10 mg at night as needed_
  **med** `Lacosamide 50 mg at night for 1 week, then 50 mg twice daily for 1 week, then increase to 100 mg in the morning and 50 mg at night for 1 week, and thereafter 100 mg twice daily if tolerated` status=planned | ev: _Initiate lacosamide as follows: 50 mg at night for 1 week, then 50 mg twice daily for 1 week, then increase to 100 mg in_
  **sz_type** `Focal epilepsy, likely temporal lobe onset` onset=focal | ev: _Focal epilepsy, likely temporal lobe onset_
  ...
  **sz_type** `startle-triggered focal events` onset=focal | ev: _startle-triggered focal events_
  **sz_type** `focal seizures with sensory triggers` onset=focal | ev: _focal seizures with sensory triggers_
  **inv** `other` result=not_stated | ev: _stable lamotrigine serum levels previously_
  ...
  **inv** `other` result=pending | ev: _Please arrange basic metabolic panel and ECG locally before uptitration to 100 mg twice daily_
  **inv** `video_EEG` result=not_stated | ev: _If breakthrough events persist despite lacosamide, we will discuss alternative adjuncts or video-EEG monitoring._
...
**SF** gold=`unknown, 2 to 3 per cluster` | pred=`1 per 7-10 days; activity-linked bursts 2-3 per 24-48 hours` | exact=✗ monthly=✗
  ...
  **sz_type** `focal epilepsy with impaired awareness` onset=focal | ev: _He describes focal epilepsy with impaired awareness_
  **sz_type** `focal impaired-awareness seizure` onset=focal | ev: _one focal impaired-awareness seizure every 7–10 days_
  **sz_type** `possible secondary generalisation overnight` onset=focal | ev: _suggestive of possible secondary generalisation overnight_
  **inv** `MRI` result=abnormal | ev: _MRI (2021) demonstrated right mesial temporal T2/FLAIR signal change without clear volume loss_
  ...
  **inv** `EEG` result=pending | ev: _ambulatory EEG (72 hours) to capture both habitual focal events and any sleep-related activity_
...
**SF** gold=`unknown, 2 per cluster` | pred=`unknown` | exact=✗ monthly=✓
  ...
  **sz_type** `intermittent nocturnal episodes` onset=unknown | ev: _intermittent nocturnal episodes over the past three months with post-event confusion on waking and occasional tongue-bit_
  **sz_type** `focal impaired-awareness seizures` onset=focal | ev: _two focal impaired-awareness seizures_
  **inv** `EEG` result=pending | ev: _Arrange routine EEG_
  **inv** `other` result=pending | ev: _updated serum biochemistry including U&E, LFT_
  **inv** `other` result=pending | ev: _levetiracetam level_
...
**SF** gold=`unknown` | pred=`unknown` | exact=✓ monthly=✓
  **med** `sodium valproate 300 mg twice daily` status=current | ev: _Sodium valproate 300 mg twice daily_
  **med** `clobazam 5 mg at night` status=current | ev: _Clobazam 5 mg at night_
  **sz_type** `drop attacks` onset=unknown | ev: _4 drop attacks_
  **inv** `EEG` result=not_stated | ev: _Arrange ambulatory EEG within the next 6–8 weeks_
...
**SF** gold=`1 per 6 week` | pred=`1 per 6 weeks` | exact=✗ monthly=✓
  **sz_type** `Focal epilepsy, likely temporal lobe onset` onset=focal | ev: _Focal epilepsy, likely temporal lobe onset._
  **sz_type** `focal aware episodes` onset=focal | ev: _stereotyped focal aware episodes characterised by a brief rising epigastric sensation and déjà vu_
  **sz_type** `impaired awareness with automatisms` onset=focal | ev: _occasionally progressing to impaired awareness with automatisms lasting 1–2 minutes_
...
**SF** gold=`2 per 3 month` | pred=`seizure free since January 2015` | exact=✗ monthly=✗
  **sz_type** `Recurrent seizures of unclear classification` onset=unknown | ev: _Recurrent seizures of unclear classification_
  ...
  **inv** `EEG` result=abnormal | ev: _Prior EEG (2014) non-specific_
  **inv** `MRI` result=normal | ev: _MRI brain (2012) structurally normal_
...
**SF** gold=`unknown` | pred=`unknown` | exact=✓ monthly=✓
  **med** `Carbamazepine modified-release 400 mg twice daily` status=current | ev: _Carbamazepine modified‑release 400 mg twice daily_
  ...
  **sz_type** `brief clusters of spasms lasting seconds` onset=not_stated | ev: _brief clusters of spasms lasting seconds_
...
**SF** gold=`unknown` | pred=`unknown` | exact=✓ monthly=✓
  ...
  **med** `Midazolam 10 mg PRN prolonged events` status=current | ev: _Rescue: buccal midazolam 10 mg PRN prolonged events_
  **sz_type** `Focal epilepsy, likely temporal lobe onset` onset=focal | ev: _Focal epilepsy, likely temporal lobe onset_
  **sz_type** `focal tonic` onset=focal | ev: _Overall seizure pattern is best characterised as focal tonic with missed ASM doses_
  **sz_type** `aura` onset=focal | ev: _Aura/ictal description (patient-reported): “A rising wave in my stomach, a metallic taste, and sounds feel far away for _
...
**SF** gold=`seizure free for multiple month` | pred=`seizure free since April 2025` | exact=✗ monthly=✓
  **med** `Levetiracetam 1,000 mg twice daily` status=current | ev: _Levetiracetam 1,000 mg twice daily_
  **med** `Clobazam 10 mg at night as needed for clusters` status=current | ev: _Clobazam 10 mg at night as needed for clusters (not recently required)_
  **sz_type** `Focal epilepsy with impaired awareness` onset=focal | ev: _Focal epilepsy with impaired awareness_
  **sz_type** `focal to bilateral tonic–clonic evolution` onset=focal | ev: _prior nocturnal and daytime focal to bilateral tonic–clonic evolution reported in March_
  **sz_type** `clusters` onset=not_stated | ev: _Clobazam 10 mg at night as needed for clusters_
...
**SF** gold=`2 per 8 month` | pred=`1 per 8 months` | exact=✗ monthly=✗
  **med** `Levetiracetam 500 mg twice daily` status=current | ev: _Current medication: Levetiracetam 500 mg twice daily_
  **sz_type** `generalised tonic-clonic seizure` onset=generalized | ev: _generalised tonic-clonic seizure three Tuesdays ago_
  **sz_type** `absences` onset=generalized | ev: _absences as brief episodes of lost time with staring and no awareness_
  **inv** `MRI` result=normal | ev: _MRI head from 2019 reported as normal_
...
**SF** gold=`13 per 4 month` | pred=`2 per month` | exact=✗ monthly=✗
  **med** `Levetiracetam 500 mg twice daily` status=current | ev: _Levetiracetam 500 mg twice daily_
  **med** `Clonazepam 0.5 mg nocte as rescue for clusters` status=current | ev: _Clonazepam 0.5 mg nocte as rescue for clusters_
  **sz_type** `Recurrent seizures of uncertain classification` onset=unknown | ev: _Recurrent seizures of uncertain classification_
  **sz_type** `brief episodes of behavioural arrest with loss of awareness` onset=unknown | ev: _brief episodes of behavioural arrest with loss of awareness_
  **sz_type** `bilateral stiffening and jerking` onset=generalized | ev: _occasionally progressing to bilateral stiffening and jerking_
  **sz_type** `daytime collapses` onset=unknown | ev: _two daytime collapses at work_
  **inv** `EEG` result=pending | ev: _Arrange urgent EEG_
  **inv** `MRI` result=pending | ev: _MRI brain with epilepsy protocol_
  **inv** `other` result=pending | ev: _Bloods today: U&E, LFT, FBC, vitamin D_
  **inv** `other` result=not_stated | ev: _Levetiracetam level if clinically indicated_
...
**SF** gold=`3 per week` | pred=`≤3 per week` | exact=✗ monthly=✗
  **med** `Levetiracetam 500 mg bd` status=current | ev: _Current Medication:

Levetiracetam 500 mg bd_
  **sz_type** `simple partial seizures` onset=focal | ev: _≤ three simple partial seizures per week_
  **sz_type** `brief events characterised by a rising epigastric sensation, momentary déjà vu, and preserved awareness, occasionally followed by mild right-hand tingling without spread` onset=focal | ev: _brief events characterised by a rising epigastric sensation, momentary déjà vu, and preserved awareness, occasionally fo_
  **sz_type** `recurrent seizure episodes of uncertain classification` onset=unknown | ev: _Recurrent seizure episodes of uncertain classification (onset 2023)_
  **inv** `MRI` result=normal | ev: _Normal brain MRI (January 2024)_
  **inv** `EEG` result=non_diagnostic | ev: _A routine EEG (February 2024) was non-diagnostic._
  **inv** `EEG` result=not_stated | ev: _if he wishes to consider ambulatory EEG during tournament periods_
...
**SF** gold=`1 per 4 day` | pred=`1 cluster per 4 days` | exact=✗ monthly=✗
  ...
  **med** `clobazam 10 mg at night as needed for clusters` status=current | ev: _Clobazam 10 mg at night as needed for clusters_
  **sz_type** `Primary generalised epilepsy` onset=generalized | ev: _Epilepsy Diagnosis:

Primary generalised epilepsy_
  **sz_type** `clusters` onset=not_stated | ev: _Seizures remain relatively stable, typically occurring in clusters every 4 days._
  **sz_type** `aura onset` onset=not_stated | ev: _more frequent magnet swiping at aura onset_
  **inv** `other` result=normal | ev: _Battery status checked at last device clinic: satisfactory._
  **inv** `other` result=pending | ev: _Next VNS device check at routine device clinic in 3 months_
...
**SF** gold=`13 per 2 month` | pred=`13 per 2 month` | exact=✓ monthly=✓
  **med** `Carbamazepine 400 mg twice daily` status=current | ev: _Carbamazepine modified‑release 400 mg twice daily_
  **med** `Levetiracetam 500 mg twice daily` status=current | ev: _Reduce levetiracetam to 500 mg twice daily over one week_
  **med** `Lacosamide 50 mg at night for one week, then 50 mg twice daily ongoing` status=current | ev: _Begin lacosamide 50 mg at night for one week, then 50 mg twice daily ongoing_
  **sz_type** `seizures during sleep` onset=not_stated | ev: _four seizures during sleep_
  **sz_type** `seizures while awake` onset=not_stated | ev: _3 while awake_
  **sz_type** `brief episodes` onset=not_stated | ev: _occasional brief episodes_
  **inv** `other` result=not_stated | ev: _We will check U&E/LFTs in four weeks_
...
**SF** gold=`2 per 2 week` | pred=`2 per 2 weeks` | exact=✗ monthly=✓
  **med** `Levetiracetam 750 mg twice daily` status=current | ev: _Levetiracetam 750 mg twice daily_
  **med** `Lamotrigine 100 mg twice daily` status=current | ev: _Lamotrigine 100 mg twice daily_
  **sz_type** `brief events with preserved awareness followed by fatigue and mild headache` onset=unknown | ev: _Episodes are described as brief events with preserved awareness followed by fatigue and mild headache_
  **inv** `other` result=normal | ev: _Routine bloods (FBC, U&E, LFTs) from August 2025 were within normal limits._
  **inv** `MRI` result=abnormal | ev: _MRI brain (2023) unchanged from prior imaging; no acute pathology._
  **inv** `EEG` result=non_diagnostic | ev: _EEG (2022) was non-specific; no clear epileptiform discharges captured._
  **inv** `other` result=pending | ev: _Request trough levels for lamotrigine and levetiracetam at the GP practice_
...
**SF** gold=`5 to 7 per 3 week` | pred=`seizure free for 6 weeks` | exact=✗ monthly=✗
  **med** `Levetiracetam 1000 mg twice daily` status=current | ev: _Levetiracetam 1000 mg twice daily_
  **med** `Carbamazepine 400 mg twice daily` status=current | ev: _Carbamazepine (Tegretol Retard) 400 mg twice daily_
  **med** `Topiramate 75 mg morning and 100 mg evening` status=current | ev: _Topiramate 75 mg morning and 100 mg evening_
  **sz_type** `Focal epilepsy with impaired awareness` onset=focal | ev: _Focal epilepsy with impaired awareness_
  **sz_type** `stereotyped focal impaired-awareness episodes` onset=focal | ev: _stereotyped focal impaired-awareness episodes featuring a rising epigastric sensation, brief arrest of speech, right fac_
  **sz_type** `focal onset seizures` onset=focal | ev: _focal onset seizures_
  **sz_type** `generalised tonic-clonic seizure` onset=generalized | ev: _any generalised tonic-clonic seizure_
  **inv** `other` result=not_stated | ev: _Bloods: request U&E, LFT, FBC, vitamin D, and carbamazepine level via practice in the next two weeks_
  **inv** `other` result=not_stated | ev: _DEXA recheck due next year_
...
**SF** gold=`3 cluster per month, 4 to 5 per cluster` | pred=`3 cluster per month, 4-5 per cluster` | exact=✗ monthly=✗
  ...
  **med** `Lacosamide 100 mg twice daily` status=current | ev: _Lacosamide 100 mg twice daily_
  **sz_type** `Recurrent unprovoked seizures` onset=not_stated | ev: _Diagnosis: 

Recurrent unprovoked seizures; phenotype under ongoing evaluation_
  **sz_type** `nocturnal clusters` onset=not_stated | ev: _three nocturnal clusters this month; each ~4 - 5 events_
  **sz_type** `brief, stereotyped episodes` onset=not_stated | ev: _brief, stereotyped episodes occurring typically between 02:00 and 04:00_
  **inv** `MRI` result=normal | ev: _Prior MRI brain (2024) unremarkable._
  **inv** `EEG` result=non_diagnostic | ev: _Routine EEG (2024) non-diagnostic._
  ...
  **inv** `other` result=abnormal | ev: _Baseline bloods (June 2025) normal apart from mild iron deficiency now treated._
  **inv** `other` result=not_stated | ev: _Arrange serum levels and U&Es/LFTs prior to next review._
  **inv** `other` result=not_stated | ev: _Pre-clinic bloods: FBC, U&Es, LFTs, Levetiracetam and Lacosamide levels._
...
**SF** gold=`multiple per day` | pred=`unknown` | exact=✗ monthly=✗
  **med** `Levetiracetam 1 g b.d.` status=current | ev: _currently taking Levetiracetam 1 g b.d._
  **med** `Clobazam 10 mg nocte` status=current | ev: _and Clobazam 10 mg nocte_
  **sz_type** `brief episodes of loss of awareness with post-event confusion and marked fatigue` onset=unknown | ev: _brief episodes of loss of awareness with post-event confusion and marked fatigue_
  **sz_type** `sudden staring and unresponsiveness lasting under a minute, followed by headaches and irritability` onset=unknown | ev: _sudden staring and unresponsiveness lasting under a minute, followed by headaches and irritability_
  **sz_type** `convulsive episodes` onset=generalized | ev: _if any convulsive episodes occur_
  **inv** `EEG` result=not_stated | ev: _arranged urgent ambulatory EEG within the next 1–2 weeks_
  **inv** `other` result=not_stated | ev: _will request updated serum drug levels_
  **inv** `other` result=not_stated | ev: _routine bloods (U&E, LFT, FBC)_
...
**SF** gold=`seizure free for multiple month` | pred=`seizure free for several months` | exact=✗ monthly=✓
  **sz_type** `Focal epilepsy with impaired awareness seizures` onset=focal | ev: _Focal epilepsy with impaired awareness seizures_
  **sz_type** `brief episodes of behavioural arrest with automatisms and post-event confusion` onset=focal | ev: _brief episodes of behavioural arrest with automatisms and post‑event confusion lasting a few minutes_
  **sz_type** `breakthrough focal events` onset=focal | ev: _Should any breakthrough focal events occur_
  **inv** `MRI` result=normal | ev: _prior MRI unremarkable_
  **inv** `EEG` result=normal | ev: _interictal EEG previously normal_
  **inv** `other` result=not_stated | ev: _Routine bloods (lipid profile, liver function, bicarbonate) via GP every 6 months while on diet_
  **inv** `EEG` result=not_stated | ev: _Should any breakthrough focal events occur, we would reconsider pharmacological options and repeat EEG_
...
**SF** gold=`1 per month` | pred=`1 per month` | exact=✓ monthly=✓
  **sz_type** `episodes with impaired awareness and post-event confusion` onset=unknown | ev: _paroxysmal episodes with impaired awareness and post-event confusion_
  **sz_type** `prodromal "surge" without progression` onset=unknown | ev: _prodromal "surge" without progression on two occasions_
  **inv** `EEG` result=normal | ev: _routine EEG one year ago reportedly normal_
  **inv** `MRI` result=normal | ev: _MRI brain without contrast five years ago for headaches was described as normal_
  **inv** `EEG` result=not_stated | ev: _prolonged EEG monitoring with home ambulatory recording as a first step_
  **inv** `video_EEG` result=not_stated | ev: _followed by inpatient video-EEG if required_
  **inv** `MRI` result=not_stated | ev: _arrange an MRI brain with epilepsy protocol_
  **inv** `other` result=pending | ev: _requested thyroid function and metabolic profile via his GP_
